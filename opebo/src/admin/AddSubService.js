...
import { createSubService, getServices } from "./apiAdmin";
...
const AddSubService = () => {
  ...
  const clickSubmit = (event) => {
    ...
    createSubService(admin._id, token, formData).then((data) => {
      if (data.error) {
        setValues({ ...values, error: data.error });
      } else {
        setValues({
          ...
          location: "",
          loading: false,
          createdSubService: data.name,
        });
      }
    });
  };

  const newSubServiceForm = () => (
    <form
          ...
          <div className='flex flex-wrap -mx-3 mb-6'>
            <div className='w-full px-3'>
              ...
              <input
                ...
                type='text'
                placeholder='e.g. Delhi, Ludhiana, Meerut...'
  ...
  const showSuccess = () => (
    <div
      ...
      style={{ display: createdSubService ? "" : "none" }}
    >
      <h3 className='text-white'>{`${createdSubService}`} is created!</h3>{" "}
    </div>
  );
  ...
  const goNext = () => (
    <div className='my-10'>
      ...
      >
        Manage Services
        <i className='fas fa-edit ml-3' aria-hidden='true'></i>
      </Link>
    </div>
  );
  ...
  return (
    <>
      ...
      <Layout
        ...
        description={`Hey ${admin.name}, ready to add a new sub-service?`}
      >
